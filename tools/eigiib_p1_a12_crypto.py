...
from eigiib_p1_a12_common import CborTag, decode_cbor, encode_cbor
...
def sign_ed25519(data: bytes, private_key: Path, openssl: str = "openssl") -> bytes:
    with tempfile.TemporaryDirectory(prefix="eigiib-a12-sign-") as temp:
...
def verify_ed25519(
    ...
    if len(signature) != 64:
        raise ValueError("Ed25519 signature length")
    with tempfile.TemporaryDirectory(prefix="eigiib-a12-verify-") as temp:
        source = Path(temp) / "source.bin"
