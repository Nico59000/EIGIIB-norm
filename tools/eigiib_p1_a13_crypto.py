...
from eigiib_p1_a13_common import CborTag, decode_cbor, encode_cbor


def public_der(public_key: Path, openssl: str = "openssl") -> bytes:
    return subprocess.run([openssl, "pkey", "-pubin", "-in", str(public_key), "-outform", "DER"], check=True, capture_output=True).stdout


def sign_ed25519(data: bytes, private_key: Path, openssl: str = "openssl") -> bytes:
    with tempfile.TemporaryDirectory(prefix="eigiib-a13-sign-") as temp:
        source = Path(temp) / "source.bin"
        signature = Path(temp) / "signature.bin"
        source.write_bytes(data)
        subprocess.run([openssl, "pkeyutl", "-sign", "-rawin", "-inkey", str(private_key), "-in", str(source), "-out", str(signature)], check=True, capture_output=True)
...
def verify_ed25519(data: bytes, signature: bytes, public_key: Path, openssl: str = "openssl") -> None:
    if len(signature) != 64:
        raise ValueError("Ed25519 signature length")
    with tempfile.TemporaryDirectory(prefix="eigiib-a13-verify-") as temp:
        source = Path(temp) / "source.bin"
        sig = Path(temp) / "signature.bin"
        source.write_bytes(data)
        sig.write_bytes(signature)
        result = subprocess.run([openssl, "pkeyutl", "-verify", "-pubin", "-rawin", "-inkey", str(public_key), "-in", str(source), "-sigfile", str(sig)], capture_output=True)
    if result.returncode != 0:
        raise ValueError("Ed25519 verification")


def cose_sign1(payload: bytes, content_type: str, private_key: Path, public_key: Path, openssl: str = "openssl") -> bytes:
...
def verify_cose_sign1(raw: bytes, expected_payload: bytes, content_type: str, public_key: Path, openssl: str = "openssl") -> None:
    ...
    der = public_der(public_key, openssl)
    expected_headers: dict[int, Any] = {1: -8, 3: content_type, 4: hashlib.sha256(der).digest()}
