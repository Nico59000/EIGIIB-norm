#!/usr/bin/env python3
"""Closed standard-library JSON Schema subset validator for P1-A14."""
from __future__ import annotations
import argparse, json, re, sys
from pathlib import Path
from typing import Any
from eigiib_p1_a14_common import strict_json
...
def main()->int:
    ...
    try:
        ...
        names=["eigiib-p1-a14-capsule.schema.json","eigiib-p1-a14-replay-report.schema.json","eigiib-p1-a14-route-result.schema.json"]
        schemas={name:strict_json((schema_dir/name).read_bytes()) for name in names}
        validator=Validator(schemas)
        pairs=[("tests/fixtures/p1-a14/capsule.json",names[0]),("tests/fixtures/p1-a14/expected-report.json",names[1]),("tests/fixtures/p1-a14/expected-replay.json",names[2])]
