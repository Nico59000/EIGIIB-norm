#!/usr/bin/env python3
"""Materialize and replay the exact historical E15-A3 authority."""
...
TOOL_VERSION = "0.1.0"
STANDARD = "EIGIIB-E15-A4-HISTORICAL-E15-A3-REPLAY-1.0"
SOURCE_COMMIT = "f403e93dd6d1dcb058474d67f2cc7e73b8ad13bd"
...
def replay(root: Path, source_commit: str = SOURCE_COMMIT) -> dict[str, Any]:
    ...
    e15_a3_result = "non-conformant"
    e15_a3_tests_result = "non-conformant"
    ...
    if exact.returncode != 0 or exact.stdout.strip() != source_commit:
        findings.append(Finding("error", "E15A4.HISTORY.SOURCE", "", "exact E15-A3 source commit is unavailable"))
    ancestor = run_command(["git", "merge-base", "--is-ancestor", source_commit, "HEAD"], root)
    if ancestor.returncode != 0:
        findings.append(Finding("error", "E15A4.HISTORY.ANCESTRY", "", "E15-A3 source commit is not an ancestor of HEAD"))

    if not findings:
        with tempfile.TemporaryDirectory(prefix="eigiib-e15-a3-history-") as td:
            td_path = Path(td)
            archive = td_path / "e15-a3.tar"
            tree = td_path / "tree"
            tree.mkdir()
            archived = run_command(["git", "archive", "--format=tar", f"--output={archive}", source_commit], root)
            if archived.returncode != 0:
                findings.append(Finding("error", "E15A4.HISTORY.ARCHIVE", "", archived.stderr.strip()))
            else:
                try:
                    safe_extract(archive, tree)
                except Exception as exc:
                    findings.append(Finding("error", "E15A4.HISTORY.EXTRACT", "", str(exc)))

            if not findings:
                try:
                    parent_history_tool = tree / "tools/eigiib_historical_e15_a2_replay.py"
                    ...
                    if parent_report.get("overall_result") != "conformant":
                        raise RuntimeError("historical E15-A2 replay is non-conformant")
                    ...
                    e15_a2_result = "conformant" if parent_report.get("e15_a2_result") == "conformant" else "non-conformant"
                    history_rel = Path(".eigiib-runtime/historical-e15-a2-report.json")
                    history_path = tree / history_rel
                    history_path.parent.mkdir(parents=True, exist_ok=True)
                    history_path.write_text(json.dumps(parent_report, indent=2, sort_keys=True) + "\n", encoding="utf-8")
                except Exception as exc:
                    findings.append(Finding("error", "E15A4.HISTORY.E15A2", "historical-e15-a2", str(exc)))

                if historical_e14_result == e15_a1_result == e15_a2_result == "conformant":
                    try:
                        checker = tree / "tools/eigiib_publication_readback_check.py"
                        ...
                        expected = json.loads((tree / "tests/fixtures/e15-a3/expected-report.json").read_text(encoding="utf-8"))
                        if report != expected:
                            raise RuntimeError("E15-A3 report differs from frozen fixture")
                        e15_a3_result = "conformant"
                    except Exception as exc:
                        findings.append(Finding("error", "E15A4.HISTORY.E15A3", "e15-a3", str(exc)))

                try:
                    proc = run_command([sys.executable, "-m", "unittest", "-v", "tests/test_eigiib_publication_readback.py"], tree)
                    if proc.returncode != 0:
                        raise RuntimeError(proc.stderr or proc.stdout)
                    e15_a3_tests_result = "conformant"
                except Exception as exc:
                    findings.append(Finding("error", "E15A4.HISTORY.TESTS", "e15-a3-tests", str(exc)))

    overall = "non-conformant" if any(f.severity == "error" for f in findings) else "conformant"
    return {
        "tool": "eigiib-historical-e15-a3-replay",
        ...
        "e15_a3_result": e15_a3_result,
        "e15_a3_tests_result": e15_a3_tests_result,
