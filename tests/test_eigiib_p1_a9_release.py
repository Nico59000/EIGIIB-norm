...
CAP=ROOT/'tests/fixtures/p1-a9/capsule.json';REL=ROOT/'tests/fixtures/p1-a8/expected-release.json';RK=ROOT/'tests/fixtures/p1-a9/release-public-key.pem';TK=ROOT/'tests/fixtures/p1-a9/ts-public-key.pem';REG=ROOT/'tests/fixtures/p1-a9/a7.7-toolchain-policy-succession.json'
class P1A9Tests(unittest.TestCase):
 ...
 def test_a8_offline_policy_selector_is_closed(self):
  validated=validate_registry(ROOT,REG)
  pairs=[(entry['imageVersion'],entry['git']) for entry,_ in validated['policies']]
  self.assertEqual(pairs,[('20260714.173.1','git version 2.55.0.windows.2'),('20260728.188.1','git version 2.55.0.windows.3'),('20260803.193.1','git version 2.55.0.windows.3')])
  self.assertEqual(select_policy(ROOT,REG,'ubuntu-24.04','',''),(ROOT/'tests/fixtures/p1-a7/a7.7-toolchain-policy.json').resolve())
  for entry,expected_path in validated['policies']:
   self.assertEqual(select_policy(ROOT,REG,'windows-2025',entry['imageVersion'],entry['git']),expected_path)
  with self.assertRaises(ValueError):select_policy(ROOT,REG,'windows-2025','unregistered','git version unknown')
